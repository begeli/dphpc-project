...
def parse_file(filename):
    ...
    for idx in range(len(lines)):
        if "should receive the following" in lines[idx]:
...
def theory_memdep(p, dims, S):
    [S0, S1, S2, S3] = dims
    return math.ceil(p * 4.0 * 3*S0*S1*S2*S3/(S**(2/3)))

def theory_mmm(p, dims):
    [S0,S1,S2] = dims
    return math.floor(p * 4.0 * 3*(S0*S1*S2/p)**(2/3))
...
def compute_comm_nodims(np_list, dims, filestem):
    filelist = [filestem + '_' + str(n) + '.txt' for n in np_list]
    # TODO save to file
    commlist = [parse_file(filename) for filename in filelist]
    print('n_procs;Actual;Theory')
    for n, c in zip(np_list, commlist):
...
def compute_comm_nodims_memdep(np_list, dims, filestem, S):
    filelist = [filestem + '_' + str(n) + '.txt' for n in np_list]
    # TODO save to file
    commlist = [parse_file(filename) for filename in filelist]
    print('n_procs;Actual;Theory')
    for n, c in zip(np_list, commlist):
        print(str(n) + ';' + str(c) + ';' + str(theory_memdep(n, dims, S)))
    pass



def compute_comm_nodims_mmm(np_list, dims, filestem):
    filelist = [filestem + '_' + str(n) + '.txt' for n in np_list]
    # TODO save to file
    commlist = [parse_file(filename) for filename in filelist]
    print('n_procs;Actual;Theory')
    for n, c in zip(np_list, commlist):
        print(str(n) + ';' + str(c) + ';' + str(theory_mmm(n, dims)))
    pass



def compute_increasing(np, dimslist, filestem):
    filelist = [filestem + '_' + str(d) + '.txt' for d in dimslist]
    print('Weak scaling for ' + str(np) + ' cores.')
    commlist = [parse_file(filename) for filename in filelist]
    print('dims;actual;theory')
    for d, c in zip(dimslist, commlist):
        print(str(d) + ';' + str(c) + ';' + str(theory(np, [d, d, d, d])))
    pass


def compute_increasing_mmm(np, dimslist, filestem):
    filelist = [filestem + '_' + str(d) + '.txt' for d in dimslist]
    print('Weak scaling for ' + str(np) + ' cores.')
    commlist = [parse_file(filename) for filename in filelist]
    print('dims;actual;theory')
    for d, c in zip(dimslist, commlist):
        print(str(d) + ';' + str(c) + ';' + str(theory_mmm(np, [d, d, d])))
    pass
...
if __name__ == '__main__':
    #print(parse_counters(os.path.join(os.path.dirname(os.path.realpath(__file__)), 'bla.txt')))
    #filestem = os.path.join(os.path.dirname(os.path.realpath(__file__)), '__mmm_scd')
    #filestem = os.path.join(filestem, '__tc_scd')
    #compute_comm([i for i in range(50,1750,50)], [1024,1,1,1024], filestem)
    #precompute('ij,jk->ik', [8, 27, 64, 125, 216, 343, 512, 729, 1000, 1331, 1728], [[1024, 1024, 1024]], '__mmm_scd')
    filestem = os.path.join(os.path.dirname(os.path.realpath(__file__)), '__skinny_MMM')
    compute_comm_nodims_mmm(range(50, 1001, 50), [1, 16777216, 1], filestem)
    #compute_increasing(1000, range(500, 10001, 500), filestem)
    #compute_increasing_mmm(1000, range(500, 10001, 500), filestem)
    exit(0)
